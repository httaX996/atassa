...
gmd(
    {
        pattern: "fb",
        category: "downloader",
        react: "🧩", // ඔබගේ කෝඩ් එකේ තිබූ රියැක්ෂන් එක
        aliases: ["fbdl", "facebookdl", "facebook"],
        description: "Download Facebook videos",
    },
    async (from, Gifted, conText) => {
        ...
        if (!q) {
            await react("❌");
            return reply("❌ Please provide a valid URL."); //[span_2](start_span)[span_2](end_span)
        }
        ...
        try {
            await react("💡"); //[span_3](start_span)[span_3](end_span)
            
            // xaviabot ඩවුන්ලෝඩර් එකෙන් දත්ත ලබා ගැනීම
            const result = await getFBInfo(q); //[span_4](start_span)[span_4](end_span)
            ...
            const hd_video = result.hd;
            const sd_video = result.sd;
            const dateNow = Date.now();

            // ඔබගේ ෆයිල් එකේ තිබූ කැප්ෂන් විලාසය (Caption Style)
...
`; //[span_5](start_span)[span_5](end_span)
            ...
            // බටන් මැසේජ් එක යැවීම (කැප්ෂන් එක ඔබගේ විදිහටම එකතු කර ඇත)
            await sendButtons(Gifted, from, {
                title: `${botName} FACEBOOK DOWNLOADER`,
                text: captionHeader, // ඔබගේ Caption එක මෙතනට දැම්මා
                footer: botFooter,
                image: { url: result.thumbnail || "https://placeholder.com/fb.jpg" }, //[span_6](start_span)[span_6](end_span)
                buttons: buttons,
            });
            ...
            const handleResponse = async (event) => {
                ...
                await react("⬇️"); //[span_7](start_span)[span_7](end_span)

                try {
                    // 1. Audio ඩවුන්ලෝඩ් කිරීමේ කොටස
                    if (selectedButtonId.startsWith("fb_audio")) {
                        const sourceVideo = hd_video || sd_video;
                        if (!sourceVideo) {
                            await react("❌");
                        ...
                        await react("⬆️"); //[span_8](start_span)[span_8](end_span)
                        await Gifted.sendMessage(
                            from,
                            {
                                audio: audioBuffer,
                    ...
                    // 2. Video (HD / SD) ඩවුන්ලෝඩ් කිරීමේ කොටස
                    else {
                        ...
                        await react("⬆️"); //[span_10](start_span)[span_10](end_span)
                        
                        // ඔබගේ වීඩියෝ කැප්ෂන් එක: "> 👨🏻‍💻 *ᴄʜᴇᴛʜᴍɪɴᴀ ᴋᴀᴠɪꜱʜᴀɴ*"
                        await Gifted.sendMessage(
                            from,
                            {
                                video: { url: selectedVideoUrl }, //[span_11](start_span)[span_11](end_span)
                                mimetype: "video/mp4",
                                caption: "> 👨🏻‍💻 *ᴄʜᴇᴛʜᴍɪɴᴀ ᴋᴀᴠɪꜱʜᴀɴ*", //[span_12](start_span)[span_12](end_span)
                            },
                            { quoted: messageData },
                        );
                    }

                    await react("✅"); //[span_13](start_span)[span_13](end_span)
                } catch (error) {
